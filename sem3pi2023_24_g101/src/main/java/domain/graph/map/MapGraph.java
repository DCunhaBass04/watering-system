...
public class MapGraph<V, E> extends CommonGraph<V, E> {
    ...
    public Collection<V> adjVertices(V vert) {
        ...
        for (Map.Entry<V,MapVertex<V,E>> entry : mapVertices.entrySet()) {
            if(entry.getKey().equals(vert)) {
                outVertices.addAll(entry.getValue().getAllAdjVerts());
                break;
            }
        }
        return outVertices;
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        ...
        for(Map.Entry<V, MapVertex<V,E>> entry : mapVertices.entrySet()){
            for(Edge<V,E> edge : entry.getValue().getAllOutEdges())
